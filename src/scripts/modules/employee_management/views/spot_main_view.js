define([
  ...
  function($, _, Backbone, React, ReactDOM, BaseView, SittingPlans, SpotMain) {
    'use strict';
    var SpotMainView = BaseView.extend({
      el: '#main',
      tagName: 'div',
      className: 'spot_main_view',
      //dummy data only to be used when backend fails
      dummy_data: [
        {
            table: 1,
            project: 'Supply',
      ...
      render: function() {
        ...
        $('#loading').show();
        sittingCollection.fetch({
          complete: function () {
            $('#loading').hide();
            sittingCollection.set_model_collection();
            ReactDOM.render(_mainComponent({collection: sittingCollection}), _this.el);
          },

          error: function (error, response) {
            //in case backend fails use dummy data to show UI
            sittingCollection.reset(_this.dummy_data);
          }
        });
      }
    });
